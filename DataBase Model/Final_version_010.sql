...
CREATE TABLE IF NOT EXISTS `ambardb_model`.`buyedproducts` (
  `prodBuyed_id` INT(11) NOT NULL AUTO_INCREMENT,
  `prod_status` VARCHAR(15) NULL DEFAULT NULL,
  `prod_id` INT(11) NULL DEFAULT NULL,
  PRIMARY KEY (`prodBuyed_id`),
  INDEX `prod_buyed` (`prod_id` ASC) VISIBLE,
  CONSTRAINT `prod_buyed`
    FOREIGN KEY (`prod_id`)
...
CREATE TABLE IF NOT EXISTS `ambardb_model`.`colors` (
  ...
  UNIQUE INDEX `color_name` (`color_name` ASC) VISIBLE)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8
COLLATE = utf8_bin;
...
CREATE TABLE IF NOT EXISTS `ambardb_model`.`store` (
  ...
  INDEX `store_address` (`address_id` ASC) VISIBLE,
  INDEX `fk_store_product1_idx` (`product_prod_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `ambardb_model`.`user` (
  ...
  INDEX `usr_detail` (`usrDetail_id` ASC) VISIBLE,
  INDEX `fk_user_product1_idx` (`product_prod_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `ambardb_model`.`product_has_maker` (
  ...
  INDEX `fk_product_has_maker_product1_idx` (`product_prod_id` ASC) VISIBLE,
