...
CREATE TABLE IF NOT EXISTS `ambardb_model`.`product` (
  ...
  `prod_stockAmount` VARCHAR(2) NOT NULL DEFAULT 0,
  `prod_price` VARCHAR(7) NOT NULL DEFAULT 0,
  `prod_discount` VARCHAR(3) NULL DEFAULT 0,
  `prod_shippingfee` VARCHAR(45) NOT NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS `ambardb_model`.`user` (
  ...
  `usr_cash` VARCHAR(45) NOT NULL DEFAULT 0,
  PRIMARY KEY (`usr_id`),
...
DELIMITER $$
USE `ambardb_model`$$
CREATE PROCEDURE `sellTransaction` (prodId INT , ownerId INT, buyerId INT,storeId INT, orderStatus INT(1), orderType INT(1), storeId INT)
BEGIN
	START TRANSACTION;
